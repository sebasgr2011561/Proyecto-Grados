...
import { SortColumn, SortDirection } from './category-sortable.directive';
...
function sort(countries: CategoryModel[], column: SortColumn, direction: string): CategoryModel[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: CategoryModel, term: string, pipe: PipeTransform) {
  return country.id.toLowerCase().includes(term.toLowerCase())
    ...
    || country.by.toLowerCase().includes(term.toLowerCase())
    || country.in.toLowerCase().includes(term.toLowerCase())
    || country.price.toLowerCase().includes(term.toLowerCase())
    || country.sale.toLowerCase().includes(term.toLowerCase());
}
